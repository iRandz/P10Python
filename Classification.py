...
def myFit(classifier, x_train, y_train, x_test, y_test, y_t, y_p):
	...
	score, y_true, y_predict = Functions.validate_classification_model(classifier, x_test, y_test, False)

	y_t = np.append(y_t, y_true)
	y_p = np.append(y_p, y_predict)

	return score, y_t, y_p
...
def classify(data_features, data_labels, print_stuff, settingsIn: Settings.Settings):
	...
	for train_idx, test_idx in sgkf.split(data_features, data_labels, settingsIn.groups):
		...
		neighList[i], y_true_neigh, y_predict_neigh = myFit(neigh, x_train, y_train, x_test, y_test, y_true_neigh, y_predict_neigh)
		neuralList[i], y_true_neural, y_predict_neural = myFit(neural, x_train, y_train, x_test, y_test, y_true_neural, y_predict_neural)
		ovrList[i], y_true_ovr, y_predict_ovr = myFit(ovr, x_train, y_train, x_test, y_test, y_true_ovr, y_predict_ovr)
		svmList[i], y_true_svm, y_predict_svm = myFit(svm_clf, x_train, y_train, x_test, y_test, y_true_svm, y_predict_svm)
